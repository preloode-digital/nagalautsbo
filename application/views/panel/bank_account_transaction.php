		...
		if($value['type'] == 'Withdraw' || $value['type'] == 'Inject' || $value['type'] == 'Saving' || $value['type'] == 'Expense' || $value['type'] == 'Adjustment') {
			...
			if(!empty($data['bank']['account']['data'])) {
				...
				for($i = 0; $i < count($data['bank']['account']['data']); $i++) {
					...
					if(!empty($value['from_bank_account_id']) && $data['bank']['account']['data'][$i]['id'] == $value['from_bank_account_id']) {
...
if(!empty($data['bank']['account']['data'])) {
	...
	foreach($data['bank']['account']['data'] as $value) {
		
		$view['option']['bank']['account'] .= '<option value="'.$value['id'].'">'.$value['name'].'</option>';
...
<div id="content">
	<div class="wrapper">
        <div class="header">
        	<h2 class="page-title"><i class="transaction-white square-30 margin-right-10"></i>Bank Account Transaction</h2>
        ...
        <div class="filter">
            <form method="post" action="">
                ...
                <select name="from-bank-account">
                    ...
                    <?php echo $view['option']['bank']['account'] ?>
                </select>

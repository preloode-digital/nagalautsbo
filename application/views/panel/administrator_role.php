...
if(!empty($data['administrator']['role']['data'])) {
	...
	foreach($data['administrator']['role']['data'] as $key => $value) {
		...
		$view['administrator']['role']['list'] .= '<tr>
			<td><p>'.$value['id'].'</p></td>
			<td class="load-detail" data-administrator-role-id="'.$value['id'].'"><p>'.$value['name'].'</p></td>
			<td><p>'.$value['status'].'</p></td>
			<td>
				<a href="'.$this->config->item('panel_url').'administrator_role_entry/'.$value['id'].'/"><p class="action"><i class="pencil-white square-15 margin-right-10"></i>Edit</p></a>
				<button class="delete action" data-administrator-role-id="'.$value['id'].'"><i class="trash-white square-15 margin-right-10"></i>Delete</button>
			</td>
		</tr>';
...
<div id="content">
	<div class="wrapper">
        <div class="header">
        	<h2 class="page-title"><i class="user-role-white square-30 margin-right-10"></i>Administrator Role</h2>
            <div class="pagination">
            	<?php echo $data['administrator']['role']['pagination'] ?>
                <div class="clearfix"></div>
            </div>
            <form method="post" action="">
            	<button class="refresh" name="refresh" type="submit"><i class="refresh-white square-15 margin-right-10"></i>Refresh</button>
            </form>
            <a class="add-new" href="<?php echo $this->config->item('panel_url').'administrator_role_entry/' ?>"><p><i class="add-white square-15 margin-right-10"></i>Add New</p></a>
            <div class="clearfix"></div>
        </div>
        <div class="filter">
        	<form method="post" action="">
        ...
        <div class="content">
        	<table>
                ...
                <?php echo $view['administrator']['role']['list'] ?>
            </table>
            <div class="pagination">
            	<?php echo $data['administrator']['role']['pagination'] ?>
                <div class="clearfix"></div>

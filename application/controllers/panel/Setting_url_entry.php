...
class Setting_url_entry extends CI_Controller {
	...
	private function load() {
		...
		$this->load->view('panel/setting_url_entry', $data);
	...
	private function loadURL() {
		...
		if(!empty($this->uri->segment(3))) {
			...
			$this->initialize['url']['id'] = preg_replace('#[^0-9]#', '', $this->uri->segment(3));
			
		}
		
		$load = array(
			'id' => $this->initialize['url']['id']
		);
		$this->initialize['url']['data'] = $this->url_data->loadBindId($load);
		
		if(!empty($this->uri->segment(3)) && empty($this->initialize['url']['data'])) {

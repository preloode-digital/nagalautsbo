...
class Administrator_ajax extends CI_Controller {
	...
	public function __construct() {
		...
		$this->initialize = array(
			...
			'response' => array(
				'response' => '<p>Administrator process failed!</p>',
				'result' => false
			)
		);
	...
	public function index() {
		...
		if($_POST['action'] == 'deleteAdministrator') {
			
			$this->deleteAdministrator($_POST);
			
		}
		
		else if($_POST['action'] == 'loadAdministratorDetail') {
			
			$this->loadAdministratorDetail($_POST);
	...
	private function deleteAdministrator($data) {
			...
			if(!empty($this->initialize['administrator']['account'][0]['privilege_administrator'])) {
				...
				if($privilege[3] > 0) {
					...
					if(!empty($administrator)) {
						...
						$this->administrator_data->delete($delete);
						
						$this->initialize['response'] = array(
							'response' => '<p>Administrator successfully deleted!</p>',
							'result' => true
						);
	...
	/* Index children method */
	private function loadAdministratorDetail($data) {
			...
			if(!empty($this->initialize['administrator']['account'][0]['privilege_administrator'])) {
				...
				if($privilege[0] > 0) {
					...
					if(!empty($administrator)) {
						...
						$load = array(
							'id' => $administrator[0]['administrator_role_id']
						);
						...
						if(!empty($administratorRole)) {
							...
							$role = $administratorRole[0]['name'];
							
						}
						
						$administrator[0]['timestamp'] = new DateTime($administrator[0]['timestamp']);
						
						$privilege = array(
							'administrator' => str_split($administrator[0]['privilege_administrator']),
							'bank' => str_split($administrator[0]['privilege_bank']),
							'bankAccount' => str_split($administrator[0]['privilege_bank_account']),
							'blog' => str_split($administrator[0]['privilege_blog']),
							'gallery' => str_split($administrator[0]['privilege_gallery']),
							'game' => str_split($administrator[0]['privilege_game']),
							'player' => str_split($administrator[0]['privilege_player']),
							'promotion' => str_split($administrator[0]['privilege_promotion']),
							'report' => str_split($administrator[0]['privilege_report']),
							'setting' => str_split($administrator[0]['privilege_setting']),
							'transaction' => str_split($administrator[0]['privilege_transaction'])
						);
						...
						if($privilege['blog'][1] > 0) {
							...
							$privilege['blog']['insert'] = 'checked';
							
						}
						
						else {
							
							$privilege['blog']['insert'] = '';
							
						}
						
						if($privilege['blog'][2] > 0) {
							
							$privilege['blog']['edit'] = 'checked';
						...
						if($privilege['blog'][3] > 0) {
							...
							$privilege['blog']['delete'] = 'checked';
							
						}
						
						else {
							
							$privilege['blog']['delete'] = '';
							
						}
						
						if($privilege['gallery'][0] > 0) {
							
							$privilege['gallery']['view'] = 'checked';
							
						}
						
						else {
							
							$privilege['gallery']['view'] = '';
							
						}
						
						if($privilege['gallery'][1] > 0) {
							
							$privilege['gallery']['insert'] = 'checked';
							
						}
						
						else {
							
							$privilege['gallery']['insert'] = '';
							
						}
						
						if($privilege['gallery'][2] > 0) {
							
							$privilege['gallery']['edit'] = 'checked';
							
						}
						
						else {
							
							$privilege['gallery']['edit'] = '';
							
						}
						
						if($privilege['gallery'][3] > 0) {
							
							$privilege['gallery']['delete'] = 'checked';
							
						}
						
						else {
							
							$privilege['gallery']['delete'] = '';
						...
						$this->initialize['response'] = array(
							...
							<p class="colon">:</p>
							<p class="detail">'.$administrator[0]['id'].'</p>
							<div class="clearfix"></div>
							<p class="title">Username</p>
							<p class="colon">:</p>
							<p class="detail">'.$administrator[0]['username'].'</p>
							<div class="clearfix"></div>
							<p class="title">First Name</p>
							<p class="colon">:</p>
							<p class="detail">'.$administrator[0]['first_name'].'</p>
							<div class="clearfix"></div>
							<p class="title">Middle Name</p>
							<p class="colon">:</p>
							<p class="detail">'.$administrator[0]['middle_name'].'</p>
							<div class="clearfix"></div>
							<p class="title">Last Name</p>
							<p class="colon">:</p>
							<p class="detail">'.$administrator[0]['last_name'].'</p>
							<div class="clearfix"></div>
							...
							<p class="colon">:</p>
							<p class="detail">'.$administrator[0]['status'].'</p>
							<div class="clearfix"></div>
							<p class="title">Created Date</p>
							<p class="colon">:</p>
							<p class="detail">'.$administrator[0]['timestamp']->format('j-m-Y H:i:s').'</p>
							<div class="clearfix"></div>
							<h3>Privilege</h3>
							...
							<div class="checkbox">
								<div class="item">
									<input name="privilege-blog" type="checkbox" '.$privilege['blog']['view'].'>
									<p class="checkbox">View</p>
								</div>
								<div class="item">
									<input name="privilege-blog" type="checkbox" '.$privilege['blog']['insert'].'>
									<p class="checkbox">Add</p>
								</div>
								<div class="item">
									<input name="privilege-blog" type="checkbox" '.$privilege['blog']['edit'].'>
									<p class="checkbox">Edit</p>
								</div>
								<div class="item">
									<input name="privilege-blog" type="checkbox" '.$privilege['blog']['delete'].'>
									<p class="checkbox">Delete</p>
								</div>
							</div>
							<div class="clearfix"></div>
							...
							<div class="checkbox">
								<div class="item">
									<input name="privilege-gallery" type="checkbox" '.$privilege['gallery']['view'].'>
									<p class="checkbox">View</p>
								</div>
								<div class="item">
									<input name="privilege-gallery" type="checkbox" '.$privilege['gallery']['insert'].'>
									<p class="checkbox">Add</p>
								</div>
								<div class="item">
									<input name="privilege-gallery" type="checkbox" '.$privilege['gallery']['edit'].'>
									<p class="checkbox">Edit</p>
								</div>
								<div class="item">
									<input name="privilege-gallery" type="checkbox" '.$privilege['gallery']['delete'].'>
									<p class="checkbox">Delete</p>
								</div>
							</div>
							<div class="clearfix"></div>

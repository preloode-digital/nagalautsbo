...
class Setting_rule_ajax extends CI_Controller {
	...
	public function __construct() {
		...
		$this->initialize = array(
			...
			'response' => array(
				'response' => '<p>Rule failed processed!</p>',
				'result' => false
			)
		);
	...
	public function index() {
		...
		if($_POST['action'] == 'updateRule') {
			
			$this->updateRule($_POST);
	...
	/* Index children method */
	private function updateRule($data) {
					...
					if(!empty($page)) {
						...
						if(!empty($url)) {
							...
							$this->initialize['response'] = array(
								'response' => '<p>Rule successfully edited!</p>',
								'result' => true
							);

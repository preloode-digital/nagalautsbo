...
class Setting_withdrawal_ajax extends CI_Controller {
	...
	public function __construct() {
		...
		$this->initialize = array(
			...
			'response' => array(
				'response' => '<p>Withdrawal failed processed!</p>',
				'result' => false
			)
		);
	...
	public function index() {
		...
		if($_POST['action'] == 'updateWithdrawal') {
			
			$this->updateWithdrawal($_POST);
	...
	/* Index children method */
	private function updateWithdrawal($data) {
					...
					if(!empty($page)) {
						...
						if(!empty($url)) {
							...
							$this->initialize['response'] = array(
								'response' => '<p>Withdrawal successfully edited!</p>',
								'result' => true
							);

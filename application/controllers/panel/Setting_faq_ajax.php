...
class Setting_faq_ajax extends CI_Controller {
	...
	public function __construct() {
		...
		$this->initialize = array(
			...
			'response' => array(
				'response' => '<p>F.A.Q. failed processed!</p>',
				'result' => false
			)
		);
	...
	public function index() {
		...
		if($_POST['action'] == 'updateFaq') {
			
			$this->updateFaq($_POST);
	...
	/* Index children method */
	private function updateFaq($data) {
					...
					if(!empty($page)) {
						...
						if(!empty($url)) {
							...
							$this->initialize['response'] = array(
								'response' => '<p>F.A.Q. successfully edited!</p>',
								'result' => true
							);

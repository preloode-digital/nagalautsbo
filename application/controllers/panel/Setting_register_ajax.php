...
class Setting_register_ajax extends CI_Controller {
	...
	public function __construct() {
		...
		$this->initialize = array(
			...
			'response' => array(
				'response' => '<p>Register failed processed!</p>',
				'result' => false
			)
		);
	...
	public function index() {
		...
		if($_POST['action'] == 'updateRegister') {
			
			$this->updateRegister($_POST);
	...
	/* Index children method */
	private function updateRegister($data) {
					...
					if(!empty($page)) {
						...
						if(!empty($url)) {
							...
							$this->initialize['response'] = array(
								'response' => '<p>Register successfully edited!</p>',
								'result' => true
							);

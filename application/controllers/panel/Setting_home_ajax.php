...
class Setting_home_ajax extends CI_Controller {
	...
	public function __construct() {
		...
		$this->initialize = array(
			...
			'response' => array(
				'response' => '<p>Home failed processed!</p>',
				'result' => false
			)
		);
	...
	/* Index children method */
	private function updateHome($data) {
					...
					if(!empty($page)) {
						...
						if(!empty($url)) {
							...
							$this->initialize['response'] = array(
								'response' => '<p>Home successfully edited!</p>',
								'result' => true
							);

...
class Setting_url extends CI_Controller {
	...
	private function load() {
		...
		$this->load->view('panel/setting_url', $data);
	...
	private function loadFilter() {
		...
		if(!empty($_COOKIE['filter_url'])) {
			
			$filter = (array) json_decode($_COOKIE['filter_url']);
			$input = array();
	...
	/* Load children method */
	private function loadUrlPagination() {
		...
		if(!empty($this->uri->segment(3))) {
			...
			$this->initialize['url']['page'] = preg_replace('#[^0-9]#', '', $this->uri->segment(3)) - 1;
		...
		$this->initialize['url']['totalRow'] = count($this->url_data->loadIdPagination($load));
		
		$pagination = $this->config->item('pagination');
		$pagination['base_url'] = $this->config->item('panel_url').'setting_url';
		$pagination['total_rows'] = $this->initialize['url']['totalRow'];
		$this->pagination->initialize($pagination);
		$this->initialize['url']['pagination'] = $this->pagination->create_links();
		...
		$load = array(
			...
			'offset' => $pagination['per_page'] * $this->initialize['url']['page'],
			'limit' => $pagination['per_page'],
			'name' => 'filter_url'
		);
		$this->initialize['url']['data'] = $this->url_data->loadPagination($load);

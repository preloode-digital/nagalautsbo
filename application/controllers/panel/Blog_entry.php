...
class Blog_entry extends CI_Controller {
	...
	private function load() {
		...
		$this->load->view('panel/blog_entry', $data);
	...
	private function loadBlog() {
		...
		if(!empty($this->uri->segment(3))) {
			...
			$this->initialize['blog']['id'] = preg_replace('#[^0-9]#', '', $this->uri->segment(3));
			
		}
		
		$load = array(
			'id' => $this->initialize['blog']['id']
		);
		$this->initialize['blog']['data'] = $this->blog_data->loadBindId($load);
		
		if(!empty($this->uri->segment(3)) && empty($this->initialize['blog']['data'])) {
	...
	private function loadBlogCategory() {
		...
		$this->initialize['blog']['category']['data'] = $this->blog_category_data->loadOrderNameAsc();

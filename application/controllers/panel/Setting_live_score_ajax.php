...
class Setting_live_score_ajax extends CI_Controller {
	...
	public function __construct() {
		...
		$this->initialize = array(
			...
			'response' => array(
				'response' => '<p>Live score failed processed!</p>',
				'result' => false
			)
		);
	...
	public function index() {
		...
		if($_POST['action'] == 'updateLiveScore') {
			
			$this->updateLiveScore($_POST);
	...
	/* Index children method */
	private function updateLiveScore($data) {
					...
					if(!empty($page)) {
						...
						if(!empty($url)) {
							...
							$this->initialize['response'] = array(
								'response' => '<p>Live score successfully edited!</p>',
								'result' => true
							);

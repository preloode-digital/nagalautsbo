...
class Bank_entry extends CI_Controller {
	...
	private function load() {
		...
		$this->load->view('panel/bank_entry', $data);
	...
	private function loadBank() {
		...
		if(!empty($this->uri->segment(3))) {
			...
			$this->initialize['bank']['id'] = preg_replace('#[^0-9]#', '', $this->uri->segment(3));
			
		}
		
		$load = array(
			'id' => $this->initialize['bank']['id']
		);
		$this->initialize['bank']['data'] = $this->bank_data->loadBindId($load);
		
		if(!empty($this->uri->segment(3)) && empty($this->initialize['bank']['data'])) {

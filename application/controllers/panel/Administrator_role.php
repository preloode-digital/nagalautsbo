...
class Administrator_role extends CI_Controller {
	...
	private function filter($data) {
		...
		$filter = array(
			...
			'name' => 'filter_administrator_role',
	...
	private function load() {
		...
		$this->loadAdministratorRolePagination();
		...
		$this->load->view('panel/administrator_role', $data);
		
	}
	
	
	/* Load children method */
	private function loadAdministratorRolePagination() {
		...
		if(!empty($this->uri->segment(3))) {
			...
			$this->initialize['administrator']['role']['page'] = preg_replace('#[^0-9]#', '', $this->uri->segment(3)) - 1;
		...
		$this->initialize['administrator']['role']['totalRow'] = count($this->administrator_role_data->loadIdPagination($load));
		
		$pagination = $this->config->item('pagination');
		$pagination['base_url'] = $this->config->item('panel_url').'administrator_role';
		$pagination['total_rows'] = $this->initialize['administrator']['role']['totalRow'];
		$this->pagination->initialize($pagination);
		$this->initialize['administrator']['role']['pagination'] = $this->pagination->create_links();
		...
		$load = array(
			...
			'offset' => $pagination['per_page'] * $this->initialize['administrator']['role']['page'],
			'limit' => $pagination['per_page'],
			'name' => 'filter_administrator_role'
		);
		$this->initialize['administrator']['role']['data'] = $this->administrator_role_data->loadPagination($load);
	...
	private function loadFilter() {
		...
		if(!empty($_COOKIE['filter_administrator_role'])) {
			
			$filter = (array) json_decode($_COOKIE['filter_administrator_role']);
			$input = array();

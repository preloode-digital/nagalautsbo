...
class Administrator_role_entry extends CI_Controller {
	...
	private function load() {
		...
		$this->load->view('panel/administrator_role_entry', $data);
	...
	private function loadAdministratorRole() {
		...
		if(!empty($this->uri->segment(3))) {
			...
			$this->initialize['administrator']['role']['id'] = preg_replace('#[^0-9]#', '', $this->uri->segment(3));
			
		}
		
		$load = array(
			'id' => $this->initialize['administrator']['role']['id']
		);
		$this->initialize['administrator']['role']['data'] = $this->administrator_role_data->loadBindId($load);
		
		if(!empty($this->uri->segment(3)) && empty($this->initialize['administrator']['role']['data'])) {

...
class Setting_about_us_ajax extends CI_Controller {
	...
	public function __construct() {
		...
		$this->initialize = array(
			...
			'response' => array(
				'response' => '<p>About us failed processed!</p>',
				'result' => false
			)
		);
	...
	public function index() {
		...
		if($_POST['action'] == 'updateAboutUs') {
			
			$this->updateAboutUs($_POST);
	...
	/* Index children method */
	private function updateAboutUs($data) {
					...
					if(!empty($page)) {
						...
						if(!empty($url)) {
							...
							$this->initialize['response'] = array(
								'response' => '<p>About Us successfully edited!</p>',
								'result' => true
							);

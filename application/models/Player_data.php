...
class Player_data extends CI_Model {
	...
	public function loadBindStatusOrderUsernameAsc($data) {
		...
		$sql = 'SELECT * FROM player WHERE status = ? ORDER BY username ASC';
		$bind = array(
			$data['status']
		);
		$query = $this->db->query($sql, $bind);
	...
	public function loadOrderUsernameAsc() {
		...
		$sql = 'SELECT * FROM player ORDER BY username ASC';
	...
	public function uploadPicture($data) {
		...
		$upload = array(
			'upload_path' => dirname(dirname(dirname(__FILE__))).'/asset/image/player',

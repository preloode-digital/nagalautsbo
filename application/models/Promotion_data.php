...
class Promotion_data extends CI_Model {
	...
	public function loadBindName($data) {
		...
		$sql = 'SELECT * FROM promotion WHERE name = ?';
		$bind = array(
			$data['name']
		);
		$query = $this->db->query($sql, $bind);
	...
	public function loadBindStatusOrderNameAsc($data) {
		...
		$sql = 'SELECT * FROM promotion WHERE status = ? ORDER BY name ASC';
		$bind = array(
			$data['status']
		);
		$query = $this->db->query($sql, $bind);
	...
	public function loadBindStatusOrderSequenceAsc($data) {
		
		$this->load->database();
		
		$sql = 'SELECT * FROM promotion WHERE status = ? ORDER BY sequence ASC';
		$bind = array(
			$data['status']
		);
		$query = $this->db->query($sql, $bind);
	...
	public function uploadPicture($data) {
		...
		$upload = array(
			'upload_path' => dirname(dirname(dirname(__FILE__))).'/asset/image/promotion',

...
class Slider_data extends CI_Model {
	...
	public function loadBindStatusOrderSequenceAsc($data) {
		...
		$sql = 'SELECT * FROM slider WHERE status = ? ORDER BY sequence ASC';
		$bind = array(
			$data['status']
		);
		$query = $this->db->query($sql, $bind);
	...
	public function uploadPicture($data) {
		...
		$upload = array(
			'upload_path' => dirname(dirname(dirname(__FILE__))).'/asset/image/slider',

...
class Blog_data extends CI_Model {
	...
	public function loadBindCategoryIdOrderIdDesc($data) {
		
		$this->load->database();
		
		$sql = 'SELECT * FROM blog WHERE category_id = ? ORDER BY id DESC';
		$bind = array(
			$data['category_id']
		);
	...
	public function loadOrderIdDesc() {
	...
	public function uploadPicture($data) {
		...
		$upload = array(
			'upload_path' => dirname(dirname(dirname(__FILE__))).'/asset/image/blog',
